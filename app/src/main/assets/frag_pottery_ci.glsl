...
void main(){
    ...
    //computer diffuse;
    vec4 lightVector = normalize(vec4(-1,1,2,0));
    vec4 diffuse = uDiffuseL * uDiffuseM * max(dot(vec3(lightVector), vNormal), 0.0);

    vec4 lightVector2 = normalize(vec4(1.2, 0, -1, 1));
...
//        + textureCube(uCube,texCoordCube) * 0.013;
        + textureCube(uCube,texCoordCube) * 0.03;
